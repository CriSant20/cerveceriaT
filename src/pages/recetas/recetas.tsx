...
import { FiCheck, FiX, FiInfo, FiAlertTriangle, FiPlus, FiMinus } from 'react-icons/fi';
...
const IngredienteItem = ({ ingrediente }: { ingrediente: IngredienteReceta }) => (
...
const TarjetaReceta = ({ 
  ...
  const verificarIngredientes = () => {
    ...
    receta.maltas.forEach(malta => {
      const enInventario = inventario.maltas.find(m => m.nombre === malta.tipo);
    ...
    receta.lupulos.forEach(lupulo => {
      const enInventario = inventario.lupulos.find(l => l.nombre === lupulo.tipo);
    ...
    // Verificar levadura
    const enInventario = inventario.levaduras.find(l => l.nombre === receta.levadura.tipo);
    if (!enInventario || enInventario.stock < receta.levadura.cantidad) {
      faltantes.push(`${receta.levadura.tipo} (necesaria: ${receta.levadura.cantidad} kg)`);
    }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md p-4 mb-4 border-l-4 ${puedeProducir ? 'border-green-500' : 'border-red-500'}`}>
      <div className="flex justify-between items-start">
        ...
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${puedeProducir ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        <div>
          ...
          {receta.maltas.map((malta, index) => (
            <IngredienteItem key={`malta-${index}`} ingrediente={malta} />
          ))}
        </div>
        
        <div>
          <h4 className="font-semibold text-gray-700 mb-2">Lúpulos</h4>
          {receta.lupulos.map((lupulo, index) => (
            <IngredienteItem key={`lupulo-${index}`} ingrediente={lupulo} />
          ))}
        </div>
        
        <div>
          <h4 className="font-semibold text-gray-700 mb-2">Levadura</h4>
          <IngredienteItem ingrediente={receta.levadura} />
        </div>
      </div>
        ...
        <div className="mt-3 p-2 bg-red-50 text-red-700 rounded flex items-start">
          ...
          <div>
            ...
            <ul className="list-disc list-inside">
              {ingredientesFaltantes.map((item, index) => (
      ...
      <button
        ...
        className={`mt-4 w-full py-2 rounded-lg flex items-center justify-center ${puedeProducir ? 'bg-amber-600 hover:bg-amber-700 text-white' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}`}
      >
        {puedeProducir ? (
          <>
...
const Recetas = () => {
  ...
  const [recetaSeleccionada, setRecetaSeleccionada] = useState<RecetaCerveza | null>(null);
  ...
  const confirmarProduccion = () => {
    ...
    recetaSeleccionada.lupulos.forEach(lupulo => {
      const item = nuevoInventario.lupulos.find(l => l.nombre === lupulo.tipo);
      if (item) {
    ...
    // Reducir levadura
    const levadura = nuevoInventario.levaduras.find(l => l.nombre === recetaSeleccionada.levadura.tipo);
    if (levadura) {
      levadura.stock -= recetaSeleccionada.levadura.cantidad * cantidadProduccion;
    }

    setInventario(nuevoInventario);
    setMostrarModal(false);
    alert(`¡Producción de ${cantidadProduccion} lote(s) de ${recetaSeleccionada.nombre} completada!`);
  };

  return (
    <div className="space-y-6 p-4 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800">Bienvenido al Recetario</h1>
      
      <div className="bg-white rounded-lg shadow-md p-6">
        <p className="text-gray-700 mb-6">
          Selecciona una receta para producir. El sistema verificará automáticamente si tienes los ingredientes necesarios en tu inventario.
        </p>
      ...
      {mostrarModal && recetaSeleccionada && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            ...
            <p className="mb-2">Receta: <strong>{recetaSeleccionada.nombre}</strong></p>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center">
                <button 
                  onClick={() => setCantidadProduccion(Math.max(1, cantidadProduccion - 1))}
                ...
                <input
                  ...
                  onChange={(e) => setCantidadProduccion(Math.max(1, Number(e.target.value)))}
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 mb-4">
              <p className="text-yellow-700 text-sm">
                Al confirmar, se descontarán los ingredientes necesarios de tu inventario.
              </p>
            </div>
